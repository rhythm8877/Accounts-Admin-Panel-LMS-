...
import "../../styles/ResultsCSS/InternalMarks.css";
  ...
  return (
    <div className="internal-marks-container">
      <div className="marks-header">
        ...
        <p>Enter internal assessment marks for each student (Maximum: 20 marks per subject)</p>
          ...
          {subjects.map((subject) => (
            <div key={subject} className="mark-col">
              {subject === "Maths/Biology" && isHigherGrade && selectedStream === "Science" 
                ...
                : subject} <span className="max-marks">/20</span>
            </div>
          ))}
        </div>
                ...
                {subjects.map((subject) => {
                  ...
                  return (
                    <div key={`${student.id}-${actualSubject}`} className="mark-col">
                      <input
                        type="text"
                        value={internalMarks[student.id]?.[actualSubject] ?? ""}
      ...
      <div className="marks-actions">
        <button 
          ...
          disabled={disabled}
        >
          {disabled ? "Saving..." : "Save Internal Marks"}
        </button>
      </div>
    </div>
  );
